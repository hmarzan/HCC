...
test_seh PROC
		int 3
		push offset _except_handler;	;set SEH handler
		assume fs:nothing
		...
		xor eax, eax
		mov dword ptr [eax], 1	;this will raise an Access Violation Exception

		pop edx;						;remove SEH handler
...
main_ex PROC NEAR
	...
	;
	mov dword ptr [eax], 1 ;<--- the access violation is provoked here!
	;
...
;    );
	...
	;
	mov ebx, dword ptr fs:[0]  ;<--- restore the stack pointer
	mov esp, dword ptr [ebx]
	pop fs:[0]		;<--- set back the prev SEH ER pointer		
...
;
	...
	lea edx, [esp]
	push edx
	call System_GetCommandLineArgv ;TEST THIS CRITICAL AND HELPFUL H++ FUNCTION USED IN H++ PROGRAMS STARTUP	
	pop ecx
	push eax;

...
class Program
{						  			
	public static void ShowMatrixForm(double [] M, int rM, int cM)
	{	
		for(int i=0;i<rM;i++){
			print("\n| ");
			for(int j=0;j<cM;j++)
				print(M[(i*cM+j)*sizeof(double)],"  ");
			print("| ");
		}
	}
	public static double CalcTrace(double [] M, int rM, int cM)
	{
		double Tr = 0.0;	
		int n = min(rM, cM);
		for(int i=0;i< n; i++){
		printf("M[",i,",",i,"]= ", M[(i*(n+1))*sizeof(double)]);
			Tr += M[(i*(n+1))*sizeof(double)];
		}
		return Tr;
	}

	public static void Traspose(double [] M, int rM, int cM)
	{
		double []Res = new double[rM*cM];
		
		for(int i=0;i<rM;i++)
			for(int j=0;j<cM;j++)
				Res[(j*cM+i)*sizeof(double)] = M[(i*cM+j)*sizeof(double)]; //Res[j][i] = M[i][j];
	...
	public static void SumMatrices(double [] A, 
								   double [] B, 
								   int rows, 
								   int cols, 
								   OpType what)
	{
		...
		for(int i=0;i<rows;i++)
			for(int j=0;j<cols;j++){
				if(Sum==what)
					Result = A[(i*cols+j)*sizeof(double)] + B[(i*cols+j)*sizeof(double)];
				else
					Result = A[(i*cols+j)*sizeof(double)] - B[(i*cols+j)*sizeof(double)];
				
				Res[(i*cols+j)*sizeof(double)] = Result;
			}
	...
	public static void MultiplyMatrices(double [] A, 
										double [] B, 
										int rA, 
										int cA, 
										int rB, 
										int cB)
	{
		...
		if(cA==rB)
		{
			...
			for(int i=0;i<rA;i++)					
				for(int j=0;j<cB;j++){
					Res[(i*cB+j)*sizeof(double)] = 0.0;					
					printf("Res[",i,",",i,"]= ", Res[(i*cB+j)*sizeof(double)]);
				}
			...
			for(i=0;i<rA;i++)
				for(j=0;j<cA;j++)
					for(int k=0;k<cA;k++){
						//Res[i][j] +=  A[i][k] * B[k][j];
						double CVal_ij =  A[(i*cA+k)*sizeof(double)] * B[(k*cA+j)*sizeof(double)];						
						Res[(i*cB+j)*sizeof(double)] = CVal_ij + Res[(i*cB+j)*sizeof(double)];
					}					
		...
		while(true)
		{
			...
			for(int i=0;i<rA;i++)
				for(int j=0;j<cA;j++)
				{
					...
					A[(i*cA+j)*sizeof(double)] = Console::ReadDouble();
				}
			...
			for(i=0;i<rB;i++)
				for(j=0;j<cB;j++)
				{
					...
					B[(i*cB+j)*sizeof(double)] = Console::ReadDouble();
				}
